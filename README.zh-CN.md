...
- **相互隔离的 DSH 容器** —— 并排安装多个 DSH 版本，按项目创建独立容器。每个容器有自己的 profile（`web` / `headless` / 自定义）、工作区、插件集和宿主进程，实验之间不会互相污染。
- **内嵌 WebView，不需要浏览器** —— DSH 前端在 DSH Box 管理的原生 WebView 窗口里打开，不用端口转发、不用复制 URL、不用开一堆标签页。
- **容器几秒就绪** —— 拉取一个 Harness 版本时会准备好一个 base，里面**已经包含构建好的客户端产物**（native 插件、宿主/客户端库、Web 前端）。创建容器只是复制这棵树并从 pnpm store 链接依赖：实测 **~10 秒**，而在创建容器时才构建前端需要 ~195 秒。容器启动过程永远不做安装或构建。
- **插件依赖图** —— 看清一个模板、一个容器或一个正在运行的宿主实际加载了什么。每个插件是一个节点，宿主半身与浏览器半身分开绘制，节点按依赖深度分层绘制并标注把它们放到那里的 `inject`/`provide` 边，另外还有共享服务、加载顺序，以及解释「哪些没解析出来」的诊断。可以搜索、点击节点聚焦、**把某一层折叠掉**，并在详情面板里看到某个插件为什么存在。见 [架构 → 插件依赖图](#插件依赖图)。
- **容器资源：提取、注入、编辑** —— 聊天记录、provider 凭据、插件状态都抽象成带位置的 **kind**：可以从容器提取到 Box 资源库、再注入回去（默认拒绝覆盖，需要时逐条合并）、在容器之间搬运，或者打包成 tar。副本是**有名字的**，再取一份是新增而不是覆盖；一个 kind 可以同时存在于多处（provider key 既是 `.credentials.yaml` 里的 ref，也是 `settings.yaml` 里声明它的那条路由）；任意 YAML 片段都能在界面或 CLI 里**按 key 路径编辑**。插件自己声明 `dshbox.resources`、或者只是在代码里拼了个路径，也会作为候选类型出现。见 [架构 → 容器资源](#容器资源)。
- **面向 agent 的界面，不只是 GUI** —— `dshbox apply -f <file>` 吃一份文档（`container:`、`types:`、`copies:`）就让资源层与之一致；所有会汇报结果的动词都支持 `--json`；`dshbox container resource read <id> <path> --section a.b` 把容器里的文件作为 key 路径树交给 agent。见 [用 agent 驱动](#用-agent-驱动)。
- **零依赖安装** —— 每个发行版都自带一份私有的 Node、npm、pnpm 与 Git（仅 Windows）运行时。不需要系统 Node、不用配工具链、不用改 PATH。以 Git 为源的 boxfile 写法（`github.com/owner/repo:tag`）通过受管二进制在干净环境中解析——宿主 `~/.gitconfig` 永远不会泄漏进构建。Linux 上 DSH Box 使用系统 Git（`apt install git`），但配置仍然隔离在运行时目录下。
- **内置版本管理** —— 从 `deepseek-ai/deepseek-harness` 浏览 DSH 发行版，一键安装/卸载任意 tag，并按容器固定版本。
- **boxfile / 封存模板流水线** —— 用一份小型声明式脚本（`.dsh`，`FROM` + `PROFILE` + `ADD plugin|skill|data`）描述想要的容器；`dshbox build` 产出可复用的封存模板，`dshbox run <template>` 在最终容器目录里准备它一次。见 [架构 → boxfile](#boxfile-与模板构建流水线)。
- **可移植的已构建模板** —— 构建 boxfile 时把插件、技能、数据负载都物化进模板；容器拿到的是自己那份拷贝，不依赖工作区路径、pnpm 链接或可变的扩展仓库。
- **Windows 优先的运行时恢复** —— Windows 上首次运行遇到 pnpm junction 校验失败时，DSH Box 会在依赖已物化后恢复，并在启动宿主前重新分配 loopback 端口；瞬时端口绑定失败会重试而不会重建前端。
- **容器内的 agent 感知** —— DSH Box 给每个容器注入 `dsh-box-context` 插件，容器内的 agent 因此能看到 `paths.dshboxHome` 与 `paths.dshboxCli`，即使没有继承正常的 `PATH` 也能管理 DSH Box（容器、模板、插件）。
- **插件列表是「已安装」的镜像** —— 资源库只是 pnpm store 与各 profile lockfile 之上的索引，不是第二个需要手动导入的地方。boxfile 装过的插件会自己出现并标注 *自动收录*；导入一个包记录的是一条指针（`reference`，字节留在 pnpm store），本地目录或压缩包则保留为**副本**；完整整合包导出会把指针物化，使归档自洽。
- **整合包（Bundle）工作流** —— 把任意插件与技能编成一个具名包，然后两种方式导出：
  - **快速导出**：来自 GitHub 的条目保留 URL，归档很小。
  - **完整导出**：全部打包成一个可移植 `.tar.gz`。
  - 整合包可以重新导入（重名时自行选择**覆盖**或**保留**）并安装到任意容器——插件落到 profile，技能自动归类。
- **看得见的后台任务** —— 每个耗时操作（安装、启动、重建、导入、导出）都是一个可见的排队任务，带滚动日志、取消/重试/删除、历史翻页。每一步都会说明它在做什么、花了多久。
- **daemon 慢也不会冻住窗口** —— 所有访问 daemon、文件系统或外部进程的 Tauri 命令都不在主线程上执行，每个 RPC 都有超时，所以繁忙的 `dshboxd` 会回答「还没好」而不是把界面卡住。
- **双模 RPC + 实时事件流** —— daemon 是所有状态变更的唯一归属者，通过单一 `POST /rpc`（同步还是异步由 daemon 决定）和长连接 `GET /events?token=…` SSE 暴露出来。CLI、界面和外部 agent 走同一套端点；界面里没有任何业务逻辑。
- **对网络友好** —— GitHub clone 自动探测代理，可配置 GitHub 镜像与 DSH 内安装用的 npm 仓库镜像。
- **后台服务与托盘** —— 一个小的 `dshboxd` sidecar 负责常驻，托盘图标让你不用一直开着主窗口也能控制它。
- **轻量** —— 基于 Tauri，安装包小，内存占用远低于 Electron 方案。
- **双语界面** —— 英文与简体中文，可在设置里切换。
