...
- **深度分层、加载顺序、以及折叠。** 节点深度 = 它注入的最深依赖 + 1（0 表示无需等待），层内按被依赖数排序。一层装不下的深度**默认就是折叠的**——175 个节点的容器打开是 31 个盒子——点层带标签或汇总节点即可展开/折叠：该层的边改指到汇总节点、其余重新排布，被折叠的层留在画布上方当一个可展开的 chip。有环也不会掩盖顺序：仍在环里的节点标记为挂起。
