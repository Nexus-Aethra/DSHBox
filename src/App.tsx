...
import type { BoxConfig, ContainerExtensions, DshContainer, DshVersion, Language, RepositoryExtension, ServerServiceStatus, TaskRecord, ToolchainStatus, WorkspaceExtension } from './shared/types/domain'
...
const COPY = {
  en: {
    ...
    containerDetails: 'Container details', back: 'Back', activeProfile: 'Active profile', profiles: 'Profiles', addProfile: 'Add profile', plugins: 'Plugins', skills: 'Skills', logs: 'Logs', hostLog: 'Host', rebuildLog: 'Rebuild', webviewLog: 'WebView', logRefresh: 'Refresh', workspace: 'Workspace', noPlugins: 'No enabled plugins in this profile.', noSkills: 'No container skills.', containerSkill: 'Container Skill', diagnostics: 'Diagnostics', version: 'DSH version', path: 'Path', addExtension: 'Add', upgrade: 'Upgrade', scanWorkspace: 'Scan workspace', importWorkspace: 'Import to repo', extensionSource: 'GitHub URL or local tarball path', browseArchive: 'Choose tarball', adding: 'Queued…',
    pluginRepo: 'Plugin Repository', pluginRepoNote: 'Import plugins, export a tarball, or make them available to your containers.', noRepositoryPlugins: 'No imported plugins yet.', exportPlugin: 'Export tarball', installTo: 'Install to', profile: 'Profile', exporting: 'Exporting…',
...
export function App() {
  ...
  useEffect(() => {
    ...
    const refreshForTask = (task: TaskRecord) => {
      ...
      if ((task.kind === 'container-extension-add' || task.kind === 'container-extension-copy') && selectedContainer !== null && task.resourceKeys.includes(`container:${selectedContainer.id}`)) {
        void refreshSelectedContainer(selectedContainer.id).catch((reason: unknown) => { setError(String(reason)) })
      }
      if (task.kind === 'repository-extension-import' || task.kind === 'repository-extension-export' || task.kind === 'workspace-extension-import') void loadPluginRepository()
    }
  ...
  async function loadPluginRepository(): Promise<void> { try { const snapshot = await boxApi.listResourceStates(); setRepositoryDetails(snapshot.extensionRepository); setContainers(snapshot.containers) } catch (reason) { setError(String(reason)) } }
  ...
  async function showContainerDetails(container: DshContainer): Promise<void> {
    ...
    try { const [details, snapshot] = await Promise.all([boxApi.getContainerDetails(container.id), boxApi.listResourceStates()]); setContainerDetails(details); setRepositoryDetails(snapshot.extensionRepository); setError(null) } catch (reason) { setError(String(reason)) }
  }
  ...
  async function addContainerExtension(profile: string | null, repositoryId: string): Promise<void> {
    if (selectedContainer === null) return
    try { await boxApi.enqueueContainerExtensionCopy(selectedContainer.id, profile, repositoryId); setError(null) } catch (reason) { setError(String(reason)) }
  }
  async function deleteContainerPlugin(profile: string, name: string): Promise<void> {
    if (selectedContainer === null) return
    try { await boxApi.removeRepositoryPlugin(selectedContainer.id, profile, name); await refreshSelectedContainer(selectedContainer.id); setError(null) } catch (reason) { setError(String(reason)) }
  }
  async function scanWorkspaceExtensions(): Promise<void> {
    if (selectedContainer === null) return
    try { setWorkspaceExtensions(await boxApi.scanContainerWorkspaceExtensions(selectedContainer.id)); setError(null) } catch (reason) { setError(String(reason)) }
  }
  async function importWorkspaceExtension(relativePath: string): Promise<void> {
    if (selectedContainer === null) return
    try { await boxApi.enqueueWorkspaceExtensionImport(selectedContainer.id, relativePath); setError(null) } catch (reason) { setError(String(reason)) }
  }
  ...
  async function importRepositoryPlugin(source: string): Promise<void> { try { await boxApi.enqueueRepositoryExtensionImport(source); setError(null) } catch (reason) { setError(String(reason)) } }
  async function exportRepositoryPlugin(entry: RepositoryExtension): Promise<void> { try { const selected = await boxApi.choosePluginExport(text.exportPlugin, `${entry.name.replaceAll('/', '-')}.tar.gz`); if (typeof selected === 'string') await boxApi.enqueueRepositoryExtensionExport(entry.id, selected); setError(null) } catch (reason) { setError(String(reason)) } }
  async function deleteRepositoryPlugin(id: string): Promise<void> { try { await boxApi.removeRepositoryExtension(id); await loadPluginRepository(); setError(null) } catch (reason) { setError(String(reason)) } }
  ...
  return <main className="app-shell">
    ...
    {section === 'container' && selectedContainer !== null && <ContainerDetails container={selectedContainer} details={containerDetails} repository={repositoryDetails} workspaceExtensions={workspaceExtensions} text={text} onBack={() => { setSelectedContainer(null); setContainerDetails(null); setWorkspaceExtensions([]) }} onAddProfile={addContainerProfile} onSelectProfile={setContainerProfile} onAddExtension={addContainerExtension} onDeletePlugin={deleteContainerPlugin} onScanWorkspace={scanWorkspaceExtensions} onImportWorkspace={importWorkspaceExtension} onReadLog={readContainerLog} />}
    ...
    {section === 'pluginRepo' && <PluginRepo entries={repositoryDetails} text={text} onImport={importRepositoryPlugin} onChooseArchive={chooseExtensionArchive} onExport={exportRepositoryPlugin} onDelete={deleteRepositoryPlugin} />}
