...
const COPY = {
  en: {
    ...
    pluginGraphCycleHint: 'These plugins depend on each other, so no load order exists and cordis leaves them pending. A package shipping both a host and a browser half is drawn as one node, so a reported cycle can span both contexts.',
  ...
  'zh-CN': {
    ...
    pluginGraphCycleHint: '这些插件互相依赖，排不出加载顺序，cordis 会让它们保持挂起。同时包含宿主与浏览器半身的包会画成一个节点，因此这里报告的环可能跨越两种上下文。',
