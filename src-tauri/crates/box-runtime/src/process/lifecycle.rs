...
impl TrackedChild {
    pub fn wrap(child: Child) -> Self {
        Self { child: Some(child), pgid: None, reaped: false }
    }

    pub fn id(&self) -> Option<u32> { self.child.as_ref().map(Child::id) }
    pub fn pid(&self) -> Option<u32> { self.id() }
    pub fn pgid(&mut self) -> Option<i32> {
        if self.pgid.is_some() { return self.pgid; }
        let pid = self.id()?;
    ...
    pub fn unwrap(&mut self) -> std::io::Result<ExitStatus> {
        ...
        Err(std::io::Error::new(std::io::ErrorKind::Other, "child already reaped"))
    }
    ...
    pub fn wait_timeout(&mut self, timeout: Duration) -> std::io::Result<Option<ExitStatus>> {
        ...
        loop {
            match self.try_wait()? {
                Some(status) => { self.reaped = true; return Ok(Some(status)); }
                None => {
    ...
    pub fn kill_tree(&mut self, force: bool, grace: Duration) -> std::io::Result<()> {
        let pid = match self.id() { Some(pid) => pid, None => return Ok(()) };
        let pgid = self.pgid();
        let escalated = match kill_tree_pid(pid, pgid, force) {
            Ok(()) => false,
            Err(_) if !force => {
                kill_tree_pid(pid, pgid, true).is_ok()
            }
...
impl Drop for TrackedChild {
    fn drop(&mut self) {
        ...
        if let Some(pid) = self.id() {
            ...
            if let Ok(Some(_)) = self.wait_timeout(Duration::from_millis(200)) { return; }
...
pub fn probe_pid(pid: u32) -> PidState {
    #[cfg(unix)]
    {
        ...
        match std::fs::File::open(&status_path) {
            Ok(mut file) => {
                ...
                if contents.contains("State:\tZ") { PidState::Zombie }
                else { PidState::Alive }
            }
    ...
    #[cfg(windows)]
    {
        ...
        let wide: Vec<u16> = std::ffi::OsStr::new("").encode_wide().chain(std::iter::once(0)).collect();
        let _ = wide;
        ...
        if handle.is_null() {
            ...
            if ok == 0 { PidState::Unknown }
            else if exit_code == STILL_ACTIVE { PidState::Alive }
            else { PidState::Dead }
        }
    }
}
...
mod tests {
    ...
    fn wait_timeout_returns_none_for_long_running_child() {
        #[cfg(windows)]
        let child = std::process::Command::new("cmd").args(["/K", "rem keep alive"])
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
        ...
        let result = tracked.wait_timeout(Duration::from_millis(200)).expect("wait_timeout");
        #[cfg(not(windows))]
        assert!(result.is_none(), "wait_timeout must return None while child is alive");
        #[cfg(windows)]
        {
    ...
    fn wait_timeout_returns_status_for_finished_child() {
        ...
        let child = std::process::Command::new("true")
            ...
            .expect("spawn short child");
        let mut tracked = TrackedChild::wrap(child);
        let result = tracked.wait_timeout(Duration::from_secs(5)).expect("wait_timeout");
        assert!(result.is_some(), "wait_timeout must return Some(ExitStatus)");
    }
    ...
    fn probe_pid_alive_and_dead() {
        ...
        assert!(matches!(probe_pid(pid), PidState::Dead | PidState::Recycled));
    }
}