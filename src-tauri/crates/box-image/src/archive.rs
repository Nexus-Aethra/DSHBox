...
/// at the directory the archive was unpacked into; callers are expected
/// to keep it alive until they've consumed everything they need.
#[derive(Debug)]
...
/// `blobs/<digest>/source/`, skipping `.git` and `node_modules`.
pub fn write_dshimage(
    ...
    for (digest, source_root) in blobs {
        let target = PathBuf::from("blobs").join(digest).join("source");
...
mod tests {
    ...
    fn writes_and_reads_blob() {
        ...
        let blob_unpacked = out_staging.join("blobs/fnv1a64:0000000000000000/source/SKILL.md");
