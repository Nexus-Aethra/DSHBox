...
//! Thin client: every action serializes an RPC against the daemon.
...
fn create_bundle(arguments: &[String]) -> Result<(), String> {
    ...
    let value = rpc::call(
        ...
        json!({ "name": name, "repositoryIds": ids }),
    )?;
    let bundle: box_extensions::ExtensionBundle = serde_json::from_value(value)
        .map_err(|error| format!("invalid bundle from daemon: {error}"))?;
...
fn delete_bundle(name_or_id: &str) -> Result<(), String> {
    ...
    let client = rpc::connect()?;
    rpc::call(&client, "delete_extension_bundle", json!({ "id": id }))?;
...
fn load_bundle(arguments: &[String]) -> Result<(), String> {
    ...
    rpc::run_task(
        ...
        "import_bundle",
        json!({
            "archive": rpc::absolutize_path(archive),
            "conflict": conflict,
        }),
    )?;
