...
use box_extensions::{
    detect_extension_kind, extension_digest, remove_plugin_record,
    repository_root, scan_repository, scan_workspace_extensions, write_extension_record, write_repository_index,
    ExtensionKind, ExtensionRecord, RepositoryExtension,
};
use box_foundation::{
...
#[derive(Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ImportRepositoryExtensionRequest { source: String }

#[derive(Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
struct CopyRepositoryExtensionRequest { id: String, profile: Option<String>, repository_id: String }

#[derive(Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ExportRepositoryExtensionRequest { repository_id: String, destination: String }

#[derive(Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ImportWorkspaceExtensionRequest { id: String, relative_path: String }
...
fn ensure_container_workspace(directory: &Path) -> Result<PathBuf, String> {
...
#[tauri::command]
fn enqueue_repository_extension_import(
...
#[tauri::command]
fn scan_container_workspace_extensions(id: String) -> Result<Vec<box_extensions::WorkspaceExtension>, String> {
...
#[tauri::command]
fn enqueue_repository_extension_export(
...
#[tauri::command]
fn remove_repository_extension(id: String, tasks: tauri::State<TaskManager>, app: tauri::AppHandle) -> Result<(), String> {
...
fn import_repository_extension(request: ImportRepositoryExtensionRequest, task: &TaskContext) -> Result<(), String> {
    let root = read_config()?.runtime_directory.ok_or("DSH Box storage is not configured")?;
    let staging = repository_root(Path::new(&root)).join("staging").join(&task.id);
    let _ = fs::remove_dir_all(&staging);
    fs::create_dir_all(&staging).map_err(|error| error.to_string())?;
    ...
    let extracted = if source.starts_with("https://github.com/") {
        let destination = staging.join("source");
        task.log(&format!("cloning GitHub repository {source}"));
        let cancelled = task.clone(); shallow_clone_with_cancel(source, &destination, None, move || cancelled.cancelled())?; destination
    } else if Path::new(source).is_dir() {
    ...
    let destination = repository_root(Path::new(&root)).join(match kind { ExtensionKind::Plugin => "plugins", ExtensionKind::Skill => "skills" }).join(&task.id).join("source");
    ...
    entries.push(RepositoryExtension { id: task.id.clone(), kind, name, version, description, content_digest: digest, source_path: destination.to_string_lossy().into_owned(), imported_at: now_seconds(), diagnostic: None });
    write_repository_index(Path::new(&root), &entries)?;
    let _ = fs::remove_dir_all(staging);
    task.update("Repository extension imported", 95); Ok(())
}

fn import_workspace_extension(request: ImportWorkspaceExtensionRequest, task: &TaskContext) -> Result<(), String> {
    let root = read_config()?.runtime_directory.ok_or("DSH Box storage is not configured")?;
...
fn repository_metadata(kind: &ExtensionKind, source: &Path) -> Result<(String, Option<String>, Option<String>), String> {
    match kind {
        ExtensionKind::Skill => {
...
fn copy_repository_extension(request: CopyRepositoryExtensionRequest, task: &TaskContext) -> Result<(), String> {
    let root = read_config()?.runtime_directory.ok_or("DSH Box storage is not configured")?;
    ...
    let staging = PathBuf::from(&container.directory).join("extensions/staging").join(&task.id).join("source");
...
fn export_repository_extension(request: ExportRepositoryExtensionRequest, task: &TaskContext) -> Result<(), String> {
    let root = read_config()?.runtime_directory.ok_or("DSH Box storage is not configured")?;
    let entry = scan_repository(Path::new(&root)).into_iter().find(|entry| entry.id == request.repository_id).ok_or("repository extension not found")?;
    export_extension_directory(Path::new(&entry.source_path), Path::new(&request.destination), task)
}

fn export_extension_directory(source: &Path, destination: &Path, task: &TaskContext) -> Result<(), String> {
    if destination.extension().and_then(|value| value.to_str()) != Some("gz") { return Err("plugin export destination must end in .tar.gz".to_owned()); }
    fs::create_dir_all(destination.parent().ok_or("plugin export has no parent")?).map_err(|error| error.to_string())?;
    task.update("Packaging extension tarball", 30);
    let output = fs::File::create(destination).map_err(|error| format!("cannot create extension tarball: {error}"))?;
    let encoder = GzEncoder::new(output, Compression::default()); let mut archive = tar::Builder::new(encoder);
    append_plugin_archive(&mut archive, source, source, Path::new("extension"))?; archive.finish().map_err(|error| error.to_string())?;
    task.check_cancelled()?; task.update("Extension tarball exported", 95); Ok(())
}
...
fn install_container_plugin(
    ...
    let source_directory = PathBuf::from(&container.directory)
        ...
        .join(if source_kind == "repository" { source } else { &task.id })
        .join("source");
...
#[tauri::command]
fn read_container_log(id: String, log: String) -> Result<String, String> {
...
#[tauri::command]
fn append_container_webview_log(
...
fn webview_log_script(id: &str) -> String {
...
fn open_dsh_front(
    ...
    WebviewWindowBuilder::new(&app, label, WebviewUrl::External(target))
        .title("DSH")
        .initialization_script(webview_log_script(&id))
        .build()
